...
const App = ((() => {
	...
	let themes_dir = path.join(path.resolve(__dirname, '..'), 'themes');
	let themes = '';
	let program = blessed.program();

	const title = () => {
		return sprintf('Testing Blessed v%s', VERSION);
	}

	const files = glob.sync(path.join(themes_dir, '*.json'));
	for (var i = 0; i < files.length; i++) {
		let themeName = files[i].replace(themes_dir + path.sep, '').replace('.json', '');
		themes += `${themeName}|`;
	}
	...
	const drawHeader = () => {
		...
		headerText = ` {bold}${title()}{/bold}{white-fg} for ${os.hostname()} `;
		headerTextNoTags = ` ${title()} for ${os.hostname()} `;
	...
	return {
		...
		init () {
			...
			try {
				loadedTheme = require(path.join(themes_dir, `${theme}.json`));
			} catch (e) {
			...
			const setupCharts = () => {
				...
				for (const plugin in plugins) {
					...
					charts[plugin] = {
						chart: currentCanvas,
						values,
						plugin: require(`./terminal_sections/${plugins[plugin]}.js`),
						width,
						height
					}
