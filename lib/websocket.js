...
class Websocket {
	constructor() {
		...
		this.message = (data) => {
			...
			if (data.type === 'match') {
				...
				if (data.reason === 'filled' && data.side === 'buy' && this.my_buy_ids.indexOf(data.order_id)) {
	...
	addTrade(product_id, price) {
		...
		if (this.trades[product_id].length > settings.get(`${product_id}.strategies.${settings.get(`${product_id}.strategy`)}.trades_n`))
		{
