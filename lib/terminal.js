...
function account (data) {
	...
	if (data && !data.error)
	{
		...
		let updated = sprintf('%d:%d', (new Date(data.timestamp)).getHours(), (new Date(data.timestamp)).getMinutes());
		...
		options.attr = theme.account.standard;
		b_account.put(options, data.account.holds);
...
function coin (product_id, data) {
	...
	let x_cell_width = b_coin_pid.width / 6;
	...
	let buy_theme = (data.trade_enabled ? (data.should_buy ? theme.coins.buying.yes : theme.coins.buying.no) : theme.coins.buying.cant)
	...
	options.x = x_cell_width * 3;
	options.attr = theme.coins.label;
	b_coin_pid.put(options, 'Trade Size');

	options.x = x_cell_width * 4;
	options.attr = theme.coins.label;
	b_coin_pid.put(options, 'Price');

	options.x = x_cell_width * 5;
	options.attr = theme.coins.label;
	b_coin_pid.put(options, 'Time');


	// Trades
	// console.log('data.last_match: ', trades[product_id]);
	if (
		data.last_match &&
		// trades[product_id] &&
	...
	if (trades[product_id].length > 0) {
		...
		for (let i = trades[product_id].length - 1; i>=0; i--)
		{
			...
			options.y = row;
			options.attr = theme.coins.standard;
			b_coin_pid.put(options, '%d:%d', time.getHours(), time.getMinutes());
	...
	options.attr = theme.coins.standard;
	b_coin_pid.put(options, data.account.holds);

	let hold_length = hold_label.length + String(data.account.holds).length;
